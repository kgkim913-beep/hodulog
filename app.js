...
(function () {
  ...
  async function ensureAuth() {
    ...
    // 세션이 없으면(처음 접속 또는 사이트 데이터를 지운 경우) 로그인 화면을 띄우고
    // 로그인에 성공할 때까지 기다린다. 같은 이메일/비밀번호로 다시 로그인하면
    // 항상 같은 계정(uid)으로 연결되므로 기존 기록에 계속 접근할 수 있다.
    await waitForLogin();
  }
  ...
  function waitForLogin() {
    return new Promise((resolve) => {
      ...
      if (!loginEl || !emailInput || !passwordInput || !errorEl || !submitBtn) {
        resolve();
        return;
      }
      ...
      async function trySubmit() {
        ...
        if (error) {
          errorEl.textContent = '로그인에 실패했어요: ' + error.message;
          errorEl.style.display = 'block';
          return;
        }
  ...
  window.renderRecordPhotos = function renderRecordPhotosReal() {
    ...
    const newHtml = recordPhotos
      .map(
        (p, i) =>
          `<div class="photo-thumb" data-idx="${i}"><img src="${p.previewUrl}" style="width:100%;height:100%;object-fit:cover;border-radius:inherit;">` +
          `<span class="photo-save" data-idx="${i}" style="position:absolute; left:4px; bottom:4px; background:rgba(0,0,0,0.55); color:#fff; font-size:10px; padding:2px 6px; border-radius:8px;">저장</span>` +
          `<span class="photo-del" data-idx="${i}">✕</span></div>`
      )
      .join('');
    ...
    row.querySelectorAll('.photo-save[data-idx]').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const idx = parseInt(btn.dataset.idx, 10);
        const p = recordPhotos[idx];
        if (p && p.file) trySaveImageToDevice(p.file, btn);
      });
    });
  ...
  /* ---------------------------------------------------------
   * 3-C. 촬영/선택한 사진을 휴대폰 사진 앨범에 저장
   * 웹(PWA 포함)에서는 코드로 바로 "카메라 앨범에 저장"을 강제할 방법이
   * 없다. iOS/Android가 공통으로 지원하는 방식은 시스템 공유 시트를 띄워
   * 사용자가 직접 "이미지 저장"을 선택하게 하는 것(Web Share API,
   * navigator.share({files})). 이걸 지원하지 않는 브라우저에서는 대신
   * 사진을 길게 눌러 저장하는 방법을 안내한다. 어느 경우든 저장을
   * 취소/실패해도 기록 등록·서버 업로드 흐름에는 영향을 주지 않는다.
   * ------------------------------------------------------- */
  async function trySaveImageToDevice(file, btnEl) {
    if (!file) return;
    const originalText = btnEl ? btnEl.textContent : '';

    let canUseShare = false;
    try {
      canUseShare = !!(navigator.canShare && navigator.share && navigator.canShare({ files: [file] }));
    } catch (e) {
      // 일부 브라우저(특히 시크릿 모드)는 canShare 자체가 예외를 던진다.
      // 이 경우도 "지원 안 함"과 동일하게 처리해서, 사용자에게 반드시 안내가 뜨도록 한다.
      canUseShare = false;
    }

    if (canUseShare) {
      try {
        if (btnEl) btnEl.textContent = '저장 중…';
        await navigator.share({ files: [file], title: '호두로그 사진' });
        if (btnEl) {
        ...
        // 사용자가 공유 시트에서 취소한 경우도 여기로 들어온다. 정상 흐름이므로 조용히 원복만 한다.
        if (btnEl) btnEl.textContent = originalText;
        return;
      }
    }

    // Web Share API(파일 공유)를 지원하지 않거나 확인 자체가 실패한 경우
    // (크롬 시크릿 모드 등) — 항상 이 안내가 뜨도록 보장한다.
    if (btnEl) btnEl.textContent = originalText;
    alert('이 브라우저에서는 자동 저장이 지원되지 않아요.\n사진을 길게 눌러 "이미지 저장"을 선택해주세요.');
  }
  ...
  /** 1위 전용 실제 캐릭터 이미지. 2·3위는 캐릭터 없이 빈 상태로 둔다. */
  const RANK1_CHAR_IMG = '<img src="icons/rank1.png" alt="1위 호두과자 캐릭터" style="width:64px;height:64px;object-fit:contain;display:block;margin:0 auto;">';

  /** 1~3위 포디움 한 칸. record가 없어도(기록 0~2개) 자리는 항상 보여준다. */
  function podiumColHtml(record, rank) {
    const charHtml = rank === 1 ? RANK1_CHAR_IMG : '';
    if (!record) {
      return `<div class="podium-col rank-${rank}">
        ${charHtml}
        <div class="podium-name">아직 기록이 없어요</div>
        <div class="podium-route">&nbsp;</div>
  ...
  /**
   * "그 다음으로 좋았던 곳" 아래쪽에 보여줄 임시(샘플) 데이터.
   * 실제 기록/랭킹 집계에는 전혀 영향을 주지 않으며, 목록에서만 노출된다.
   * 삭제한 항목의 id는 localStorage에 저장해 새로고침해도 다시 나타나지 않는다.
   */
  const SAMPLE_RANKING_ITEMS = [
    { id: 'sample-1', place_name: '샘플휴게소 A', trip_name: '예시 여행 · 샘플 데이터', final_score: 82 },
    { id: 'sample-2', place_name: '샘플휴게소 B', trip_name: '예시 여행 · 샘플 데이터', final_score: 76 },
    { id: 'sample-3', place_name: '샘플휴게소 C', trip_name: '예시 여행 · 샘플 데이터', final_score: 69 },
  ];
  const HIDDEN_SAMPLES_KEY = 'hodulog_hidden_sample_rankings';
  function getHiddenSampleIds() {
    try {
      return JSON.parse(localStorage.getItem(HIDDEN_SAMPLES_KEY) || '[]');
    } catch (e) {
      return [];
    }
  }
  function hideSampleRankingItem(id) {
    ...
    if (!hidden.includes(id)) {
      ...
      try {
        ...
        /* 저장 실패해도 화면에서는 즉시 사라지도록 무시 */
      }
    }
  }

  let lastRankingRecords = [];

  function realRankingRowHtml(r, displayIndex) {
    ...
    return `<div class="list-item" data-real-record-id="${r.id}">
      <div style="width:20px; font-size:13px; color:var(--text-400); flex-shrink:0;">${displayIndex}</div>
  ...
  function sampleRankingRowHtml(item, displayIndex) {
    const scoreOutOf5 = ((item.final_score || 0) / 20).toFixed(1);
    return `<div class="list-item" data-sample-id="${item.id}">
      <div style="width:20px; font-size:13px; color:var(--text-400); flex-shrink:0;">${displayIndex}</div>
      <div class="thumb">사진</div>
      <div class="list-body">
        <div class="list-title">${escapeHtml(item.place_name)}
          <span style="font-size:10px; color:var(--warm-accent); border:1px solid var(--warm-accent); border-radius:6px; padding:1px 5px; margin-left:4px; vertical-align:1px;">샘플</span>
        </div>
        <div class="list-sub">${escapeHtml(item.trip_name || '')}</div>
      </div>
      <div class="list-trail" style="display:flex; align-items:center; gap:8px;">
        <span><span class="wn-static"></span> ${scoreOutOf5}</span>
        <button class="sample-del-btn" data-sample-id="${item.id}" aria-label="샘플 삭제"
          style="border:none; background:none; color:var(--text-400); font-size:15px; line-height:1; cursor:pointer; padding:4px;">✕</button>
      </div>
    </div>`;
  }
  ...
  function renderRanking(records) {
    ...
    lastRankingRecords = records || [];
    const ranked = sortForRanking(lastRankingRecords);

    // 1·2·3위는 실제 기록이 0~2개뿐이어도 항상 세 자리와 캐릭터를 모두 보여준다.
    const top3 = [ranked[0] || null, ranked[1] || null, ranked[2] || null];
    const podiumOrder = [
      { rank: 2, record: top3[1] },
      { rank: 1, record: top3[0] },
      { rank: 3, record: top3[2] },
    ];
    podiumEl.innerHTML = `<div class="podium">${podiumOrder
      .map(({ rank, record }) => podiumColHtml(record, rank))
      .join('')}</div>`;
    ...
    // "그 다음으로 좋았던 곳": 실제 4위 이후 기록 + (삭제되지 않은) 샘플 데이터
    const rest = ranked.slice(3);
    const hiddenSampleIds = getHiddenSampleIds();
    const visibleSamples = SAMPLE_RANKING_ITEMS.filter((s) => !hiddenSampleIds.includes(s.id));

    let displayIndex = 4;
    const rowsHtml = [];
    rest.forEach((r) => {
      rowsHtml.push(realRankingRowHtml(r, displayIndex));
      displayIndex += 1;
    });
    visibleSamples.forEach((s) => {
      rowsHtml.push(sampleRankingRowHtml(s, displayIndex));
      displayIndex += 1;
    });
    ...
    listEl.querySelectorAll('.sample-del-btn').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        hideSampleRankingItem(btn.dataset.sampleId);
        // 실제 기록/상위 랭킹은 건드리지 않고 목록만 다시 그린다.
        renderRanking(lastRankingRecords);
      });
    });
  }
  ...
  function renderHomeRankPreview(records) {
    ...
    const top = ranked[0];
    const charHtml = '<img src="icons/rank1.png" alt="1위 호두과자 캐릭터" style="width:100%;height:100%;object-fit:contain;">';
    const scoreOutOf5 = ((top.final_score || 0) / 20).toFixed(1);
    el.innerHTML = `<div class="featured" data-real-record-id="${top.id}">
      <div class="featured-char" style="display:flex; align-items:center; justify-content:center;">${charHtml}</div>
      <div class="list-body">
  ...
  async function loadMyRecords() {
    ...
    if (error) {
      const retryHtml = (label) =>
        `<div style="padding:16px; text-align:center;">` +
        `<div style="color:#B3432B; font-size:13px; margin-bottom:8px;">${label}</div>` +
        `<button class="btn ghost-lg" style="padding:8px 16px; font-size:13px;" onclick="loadMyRecords()">다시 시도</button>` +
        `</div>`;
      // 중요: 조회 실패는 "기록이 0개"가 아니다. 실패 상태를 빈 상태로 오인시키지 않도록
      // 홈 미리보기 / 랭킹 화면도 각각 재시도 버튼이 있는 실패 안내로 표시한다.
      listEl.innerHTML = retryHtml('불러오기 실패: ' + error.message);
      const homePreviewEl = document.getElementById('home-rank-preview');
      if (homePreviewEl) homePreviewEl.innerHTML = retryHtml('불러오기 실패');
      const podiumEl = document.getElementById('ranking-podium');
      const rankListEl = document.getElementById('ranking-list');
      if (podiumEl) podiumEl.innerHTML = retryHtml('불러오기 실패');
