...
(function () {
  ...
  const RANK1_CHAR_IMG = '<img src="icons/rank1.jpeg" alt="1위 호두과자 캐릭터" style="width:64px;height:64px;object-fit:contain;display:block;margin:0 auto;">';
  ...
  function renderHomeRankPreview(records) {
    ...
    const top = ranked[0];
    const charHtml = '<img src="icons/rank1.jpeg" alt="1위 호두과자 캐릭터" style="width:100%;height:100%;object-fit:contain;">';
  ...
  async function loadMyRecords() {
    ...
    const { data, error } = await supabase
      .from('records')
      .select('*')
      // 예전에는 .eq('user_id', currentUserId)로 "지금 세션의 익명 uid"에만 맞는 기록만 가져왔다.
      // 그런데 이 앱은 익명 로그인 세션이 브라우저 저장소에만 저장되기 때문에,
      // 사이트 데이터를 지우거나 캐시를 초기화하면 새로운 익명 uid가 발급되어
      // 예전 uid로 저장된 기록들이 "삭제된 것처럼" 화면에서 통째로 사라져 보였다.
      // (실제로는 Supabase에 그대로 남아있고, RLS 때문에 안 보였을 뿐이다.)
      // 이 앱은 한 사람만 쓰는 개인용 앱이므로 uid로 나눌 필요가 없어,
      // 필터를 제거해 어떤 익명 세션에서 만들었든 모든 기록을 항상 보이게 한다.
